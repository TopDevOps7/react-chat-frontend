...
export default function ParameterSettingsForm({ settingsData }) {
  ...
  const itemValidation = Yup.object().shape({
    ...
    pvt_message_save_hour: Yup.string().required('This field is required'),
    default_coverage: Yup.string().required('This field is required'),
    // banned_hour: Yup.string().required('This field is required')
    app_version: Yup.string().required('This field is required')
  });
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      pvt_message_save_hour: settingsData?.settings.pvt_message_save_hour || 24,
      ...
      app_version: settingsData?.settings.app_version || 1.0
    },
            ...
            <Card sx={{ p: 3 }}>
              <Stack spacing={3}>
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 3, sm: 2 }}>
                  <AutoAwesomeSharpIcon />
                  <Typography variant="body1" component="div" style={{ lineHeight: 2 }}>
                    The server saves private chat messages in the past
                  </Typography>
                  <TextField
                    {...getFieldProps('pvt_message_save_hour')}
                    variant="standard"
                    style={{ width: 60 }}
                    error={Boolean(touched.pvt_message_save_hour && errors.pvt_message_save_hour)}
                    helperText={touched.pvt_message_save_hour && errors.pvt_message_save_hour}
                  />
                  <Typography variant="body1" component="div" style={{ lineHeight: 2 }}>
                    hours.
                  </Typography>
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 3, sm: 2 }}>
                  ...
                  <TextField
                    {...getFieldProps('app_version')}
                    variant="standard"
                    style={{ width: 60 }}
                    error={Boolean(touched.app_version && errors.app_version)}
